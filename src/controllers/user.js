...
export const UserUpdatePhoto = async (req, res) => {
  try {
    ...
    const newPath = `${_id}.${photo.mimetype.substring(6)}`;
    ...
    if (user.photo !== newPath && user.photo !== 'profile.png') {
      fs.unlinkSync(path.join(__dirname, `../photos/${user.photo}`));
    }

    // We save the new photo on the server
    photo.mv(path.join(__dirname, `../photos/${_id}.${photo.mimetype.substring(6)}`));

    // We update the path of the photo in the database
    await User.updateOne({ _id }, { photo: `${_id}.${photo.mimetype.substring(6)}` });
