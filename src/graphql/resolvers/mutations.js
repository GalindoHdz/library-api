...
export const UserUpdateData = async (_, input) => {
  try {
    ...
    if (name && last_name && new_password) {
      ...
      await User.updateOne({ _id }, { password: aux_password, name, last_name });
...
export const UserRecoverAccount = async (_, input) => {
  try {
    ...
    const user = await User.findOne({ email }, { _id: 1, name: 1, last_name: 1, email: 1 });
...
export const UserLikeBook = async (_, input) => {
  try {
    ...
    // We obtain the data sent by the user
    const { _id, key, title, author, description, year_publication, cover } = input.User;
