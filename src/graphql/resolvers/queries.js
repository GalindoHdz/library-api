...
export const UserData = async (_, input) => {
  try {
    ...
    const user = await User.findOne({ _id }, { _id: 0, password: 0, creation_date: 0, __v: 0 });
  ...
  try {
    ...
    data.forEach((element) => {
      Books.push({
        ...
        cover: element.volumeInfo.imageLinks ? element.volumeInfo.imageLinks.smallThumbnail : null,
      });
    });
